...
async function refreshSnapshot(notes) {
  const serialized = `${JSON.stringify(notes, null, 2)}\n`;

  try {
    const existing = await readFile(SNAPSHOT_PATH, "utf8").catch(() => "");

    if (existing !== serialized) {
      await writeFile(SNAPSHOT_PATH, serialized);
      console.log("[backendNotes] Refreshed notes-snapshot.json — remember to commit it");
    }
  } catch (error) {
    console.warn(`[backendNotes] Could not refresh notes-snapshot.json: ${error.message}`);
  }
}
    ...
    try {
      ...
      console.warn(
        `[backendNotes] Local notes unavailable (${localError.message}), ` +
          "building from committed notes-snapshot.json"
      );
